...
html { width: 100%; max-width: 100%; overflow-x: clip; scroll-behavior: smooth; scrollbar-color: var(--acid) var(--ink); }
body { width: 100%; max-width: 100%; margin: 0; overflow-x: clip; background: var(--ink); color: var(--paper); font-family: Manrope, Arial, sans-serif; letter-spacing: 0; }
...
.layout > *, article, .article-section, .section-body, .spoiler-shell, .spoiler-content, .code-shell { min-width: 0; max-width: 100%; }
...
.code-shell { width: 100%; position: relative; margin: 24px 0; overflow: hidden; border: 1px solid var(--line); border-left: 3px solid var(--acid); background: #0b0e0b; }
...
.code-bar > span { min-width: 0; overflow-wrap: anywhere; }
...
pre { width: 100%; max-width: 100%; margin: 0; padding: 20px; overflow-x: auto; overscroll-behavior-inline: contain; -webkit-overflow-scrolling: touch; color: var(--paper); font: 11px/1.75 "IBM Plex Mono", Consolas, monospace; tab-size: 2; }
...
@media (max-width: 620px) {
  :root { --pad: 20px; }
  .site-header { width: 100%; min-height: 62px; padding-left: max(16px, env(safe-area-inset-left)); padding-right: max(16px, env(safe-area-inset-right)); gap: 12px; }
  .brand { flex: none; }
  .header-nav { position: absolute; right: max(16px, env(safe-area-inset-right)); flex: none; width: auto; min-width: 44px; }
  .header-nav .header-link:not(.primary) { display: none; }
  .header-link.primary { width: auto; min-width: 44px; padding: 0 11px; justify-content: center; overflow: hidden; font-size: 0; }
  .header-link.primary::before { content: "HOME"; font: 8px "IBM Plex Mono", monospace; }
  ...
  .layout { display: block; padding: 60px 20px 90px; }
  ...
  .port-table, .tool-table { display: block; width: 100%; max-width: 100%; overflow-x: auto; overscroll-behavior-inline: contain; -webkit-overflow-scrolling: touch; }
  .code-bar { gap: 12px; }
  pre { padding: 16px; overflow-x: hidden; white-space: pre-wrap; overflow-wrap: anywhere; word-break: break-word; font-size: 10px; }
  pre code { display: block; min-width: 0; max-width: 100%; white-space: inherit; overflow-wrap: anywhere; }
  .spoiler-shell { width: 100%; min-height: 220px; }
  .spoiler-content { width: 100%; padding: 18px; overflow: hidden; }
...
@media (max-width: 360px) {
  ...
  .header-nav { right: max(12px, env(safe-area-inset-right)); }
  .brand { gap: 8px; font-size: 15px; }
  .brand-mark { width: 32px; }
  .hero, .overview { padding-left: 16px; padding-right: 16px; }
  .hero h1 { font-size: clamp(46px, 17vw, 64px); }
  .hero-subtitle { font-size: 11px; }
  .fact-grid, .writeup-list { width: calc(100% - 32px); }
  .fact-grid { grid-template-columns: 1fr; }
  .fact { border-left: 1px solid var(--line); }
  .layout { padding-left: 16px; padding-right: 16px; }
  .writeup-row { grid-template-columns: 30px minmax(0, 1fr); gap: 12px; }
}
